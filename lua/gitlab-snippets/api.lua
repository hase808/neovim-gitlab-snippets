...
M.request = function(instance_name, endpoint, method, data)
	...
	if response.status >= 400 then -- Check for any client or server error
		...
		if response.status == 401 then
			error_message = error_message
				.. ": Unauthorized. Please check your GitLab token and its permissions (api scope)."
		elseif response.status == 403 then
			error_message = error_message
				.. ": Forbidden. You might not have access to this resource or the token lacks necessary scopes."
	...
	-- Handle non-error statuses that are not 200, 201, or 204 if necessary,
	-- though GitLab API usually sticks to these for success.
...
M.get_project_snippet = function(instance_name, project_id, snippet_id)
	return _get_snippet_metadata(
		instance_name,
		"/projects/" .. tostring(project_id) .. "/snippets/" .. tostring(snippet_id)
	)
end
